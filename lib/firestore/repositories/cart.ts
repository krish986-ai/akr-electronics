...
export class CartRepository {
  static async getOrCreate(userId: string): Promise<Cart> {
    const cartDoc = await getDoc(doc(getDb(), 'carts', userId));
    if (cartDoc.exists()) {
  ...
  static async getItems(userId: string): Promise<CartItem[]> {
    const snapshot = await getDocs(collection(getDb(), 'carts', userId, 'items'));
  ...
  static async getItem(userId: string, itemId: string): Promise<CartItem | null> {
    const doc_ = await getDoc(doc(getDb(), 'carts', userId, 'items', itemId));
  ...
  static async removeItem(userId: string, itemId: string): Promise<void> {
    await deleteDoc(doc(getDb(), 'carts', userId, 'items', itemId));
    await updateDoc(doc(getDb(), 'carts', userId), { updatedAt: new Date() });
  }

  static async clearCart(userId: string): Promise<void> {
    const items = await this.getItems(userId);
    for (const item of items) {
      await deleteDoc(doc(getDb(), 'carts', userId, 'items', item.id));
    }
    await updateDoc(doc(getDb(), 'carts', userId), { updatedAt: new Date() });
  }
